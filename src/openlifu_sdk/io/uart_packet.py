...
from .LIFUConfig import OW_END_BYTE, OW_START_BYTE
...
class OWUartPacket:
    ...
    def print_packet(self):
        if self is None:
            log.debug("Packet is None")
            return
        
        log.debug("UartPacket:")
        log.debug(f"  Packet ID: {self.id}")
        log.debug(f"  Packet Type: {hex(self.packet_type)}")
        log.debug(f"  Command: {hex(self.command)}")
        log.debug(f"  Address: {hex(self.addr)}")
        log.debug(f"  Reserved: {hex(self.reserved)}")
        log.debug(f"  Data Length: {self.data_len}")
        if self.data_len > 0:
            log.debug(f"  Data: {self.data.hex()}")
        else:
            log.debug("  Data: None")
        log.debug(f"  CRC: {hex(self.crc)}")
