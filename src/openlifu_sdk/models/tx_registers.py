...
and packing them into register maps that can be written to hardware.
"""
...
ADDRESSES_GLOBAL = [
    ADDRESS_GLOBAL_MODE, ADDRESS_STANDBY, ADDRESS_DYNPWR_2,
    ADDRESS_LDO_PWR_1, ADDRESS_TRSW_TURNOFF, ADDRESS_DYNPWR_1,
    ADDRESS_LDO_PWR_2, ADDRESS_TRSW_TURNON, ADDRESS_DELAY_SEL,
    ADDRESS_PATTERN_MODE, ADDRESS_PATTERN_REPEAT, ADDRESS_PATTERN_SEL_G1,
    ADDRESS_PATTERN_SEL_G2, ADDRESS_TRSW, ADDRESS_APODIZATION,
]
...
DELAY_CHANNEL_MAP: Dict[int, dict] = {}
for _row, _channels in enumerate(DELAY_ORDER_REVERSED):
    for _i, _channel in enumerate(_channels):
        DELAY_CHANNEL_MAP[_channel] = {"row": _row, "lsb": 16 * (1 - _i)}
...
PATTERN_MAP: Dict[int, dict] = {}
for _row, _periods in enumerate(PATTERN_PERIOD_ORDER):
    for _i, _period in enumerate(_periods):
        PATTERN_MAP[_period] = {
            "row": _row,
            "lsb_lvl": _i * (PATTERN_LEVEL_WIDTH + PATTERN_LENGTH_WIDTH),
            "lsb_period": _i * (PATTERN_LENGTH_WIDTH + PATTERN_LEVEL_WIDTH) + PATTERN_LEVEL_WIDTH,
        }
...
def _to_seconds(value: float, units: str) -> float:
    factor = _UNIT_TO_SECONDS.get(units)
    if factor is None:
        raise ValueError(f"Unknown time unit '{units}'; expected one of {list(_UNIT_TO_SECONDS)}")
    return value * factor
...
def set_register_value(reg_value: int, value: int, lsb: int = 0, width: int | None = None) -> int:
    if width is None:
...
def get_register_value(reg_value: int, lsb: int = 0, width: int | None = None) -> int:
    if width is None:
        width = REGISTER_WIDTH - lsb
    mask = (1 << width) - 1
...
# ---------------------------------------------------------------------------
# Delay / pattern location helpers
# ---------------------------------------------------------------------------

def get_delay_location(channel: int, profile: int = 1) -> tuple[int, int]:
...
def get_pattern_location(period: int, profile: int = 1) -> tuple[int, int, int]:
...
# ---------------------------------------------------------------------------
# Pulse pattern calculation
# ---------------------------------------------------------------------------

def calc_pulse_pattern(frequency: float, duty_cycle: float = DEFAULT_PATTERN_DUTY_CYCLE,
                       bf_clk: float = DEFAULT_CLK_FREQ) -> dict:
    clk_div_n = 0
    while clk_div_n < 6:
        ...
        if first_on_samples < 2:
            log.warning("Duty cycle too short. Setting to minimum of 2 samples")
            first_on_samples = 2
...
def pack_registers(regs: dict, pack_single: bool = False) -> dict:
...
def swap_byte_order(regs: dict) -> dict:
    swapped: dict = {}
...
class Tx7332DelayProfile:
    """A single delay profile for one TX7332 chip (32 channels)."""
...
class Tx7332PulseProfile:
    """A single pulse profile for one TX7332 chip."""
...
class Tx7332Registers:
    """Register state for a single TX7332 chip."""
    ...
    def get_delay_control_registers(self, profile: int | None = None) -> Dict[int, int]:
        ...
        for i, apod in enumerate(delay_profile.apodizations):
            apod_register = set_register_value(
                apod_register, 1 - apod,
                lsb=APODIZATION_CHANNEL_ORDER_REVERSED.index(i + 1), width=1,
            )
    ...
    def get_delay_data_registers(self, profile: int | None = None,
                                 pack: bool = False, pack_single: bool = False) -> Dict[int, int]:
        if profile is None:
            profile = self.active_delay_profile
        delay_profile = self.get_delay_profile(profile)
        data_registers: Dict[int, int] = {}
        for channel in range(1, NUM_CHANNELS + 1):
            ...
            delay_seconds = _to_seconds(delay_profile.delays[channel - 1], delay_profile.units)
            delay_value = int(delay_seconds * self.bf_clk)
    ...
    def get_pulse_data_registers(self, profile: int | None = None,
                                 pack: bool = False, pack_single: bool = False) -> Dict[int, int]:
        if profile is None:
            profile = self.active_pulse_profile
        ...
        nperiods = len(levels)
        level_lut = {-1: 0b01, 0: 0b11, 1: 0b10}
    ...
    def get_registers(self, profiles: ProfileOpts = "configured",
                      pack: bool = False, pack_single: bool = False,
                      pulse_invert: bool = False) -> Dict[int, int]:
...
# ═══════════════════════════════════════════════════════════════════════════
# Multi-chip device register manager
# ═══════════════════════════════════════════════════════════════════════════

@dataclass
class TxDeviceRegisters:
    """Manages register state across all TX7332 chips on a transmitter device."""
    ...
    module_invert: List[bool] | bool = False
    ...
    def recompute_delay_profiles(self):
        for tx in self.transmitters:
            for profile in tx.configured_delay_profiles():
                tx.remove_delay_profile(profile)
    ...
    def recompute_pulse_profiles(self):
        for tx in self.transmitters:
            for profile in tx.configured_pulse_profiles():
                tx.remove_pulse_profile(profile)
    ...
    def get_registers(self, profiles: ProfileOpts = "configured", recompute: bool = False,
                      pack: bool = False, pack_single: bool = False) -> List[Dict[int, int]]:
        if recompute:
        ...
        if isinstance(self.module_invert, bool):
            ...
            tx_invert = list(
                ...
                .reshape(TRANSMITTERS_PER_MODULE, -1).T.reshape(-1)
            )
        return [
            tx.get_registers(profiles, pack=pack, pack_single=pack_single, pulse_invert=inv)
            for tx, inv in zip(self.transmitters, tx_invert)
        ]
    ...
    def get_pulse_control_registers(self, profile: int | None = None) -> List[Dict[int, int]]:
        ...
        if isinstance(self.module_invert, bool):
            ...
            tx_invert = list(
                np.array(self.module_invert * TRANSMITTERS_PER_MODULE, dtype=bool).T.reshape(-1)
            )
        return [
